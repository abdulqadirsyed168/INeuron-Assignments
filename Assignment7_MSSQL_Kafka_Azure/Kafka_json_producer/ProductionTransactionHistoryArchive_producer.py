...
cnx = create_engine('mysql+pymysql://root:my-secret-pw@localhost:3366/adventure')    
query = 'select * from Production_TransactionHistoryArchive'
# df = pd.read_sql(query, cnx) #read the entire table
...
def get_car_instance(query,cnx):
    df=pd.read_sql(query,cnx)
    df[['TransactionDate','ModifiedDate']] = df[['TransactionDate','ModifiedDate']].astype(str)
    df=df.iloc[:,0:]
...
def main(topic):
  ...
  "type": "object"
    ...
    producer.poll(0.0)
    try:
        for idx,car in enumerate(get_car_instance(query,cnx)):
            ...
            # if idx==1:    # index value is used for testing 1 or 2 records            
            #     break
...
main("ProductionTransactionHistoryArchive")